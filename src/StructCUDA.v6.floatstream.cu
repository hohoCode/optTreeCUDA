//////////////////////////////Author: Hua He
//////////////////////////////OptTree CUDA Version 6 - Compression
#include <stdio.h>
#include <cuda.h>
...
////////////////////////The Compression - Move from 16 bits to 32 bits
////////////////////////This operation happens inside GPU
__device__ float slope2float( sdt_slope h ) {
...
__device__ float getLeafLoop(StructSimple* root, sdt_slope* featureS) {
	StructSimple* item = root;
	while(item->left!=0||item->right!=0){
		float cc = slope2float(*(featureS + item->fid));
...
__global__ void scoreAccumulator(StructSimple* tree, sdt_slope* feature, int* nodeSizes, int numberOfInstances, int nbTrees, int numberOfFeatures, float* output){	
...
void kernel_wrapper(sdt_slope* feature, StructSimple* tree, int* nodeSizes, int numberOfInstances, int nbTrees, int numberOfFeatures, int totalNodes){
	//////////////////CUDA Varibles Definition
	int* nodeSizes_cuda;
	StructSimple* tree_cuda;

	float* output[STREAM_COUNT];
	sdt_slope* feature_stream[STREAM_COUNT];
	...
	for( ii =0; ii<STREAM_COUNT; ++ii ) {			
		...
		if(ii == STREAM_COUNT-1){	
			...
			cudaMalloc((void**)&feature_stream[ii], sizeof(sdt_slope) * endSize);
			...
			cudaMalloc((void**)&feature_stream[ii], sizeof(sdt_slope) * SIZE);
	...
	for( ii =0; ii<STREAM_COUNT; ii+=2 ){
		int next = ii+1;
		cudaMemcpyAsync(feature_stream[ii], feature+ii*SIZE, sizeof(sdt_slope)*SIZE, cudaMemcpyHostToDevice, stream[ii]);		
		//////////////////////////The if condition is to deal with the final cuda block special situation.
		if(next == STREAM_COUNT-1){				
			cudaMemcpyAsync(feature_stream[next], feature+next*SIZE, sizeof(sdt_slope)*endSize, cudaMemcpyHostToDevice, stream[next]);	
		}else{
			cudaMemcpyAsync(feature_stream[next], feature+next*SIZE, sizeof(sdt_slope)*SIZE, cudaMemcpyHostToDevice, stream[next]);	
		}
